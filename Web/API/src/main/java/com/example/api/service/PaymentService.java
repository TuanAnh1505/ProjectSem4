...
public class PaymentService {
    ...
    public PaymentResponseDTO updatePaymentStatus(Integer paymentId, Integer statusId, String notes) {
        ...
        // Chỉ gửi email nếu trạng thái trước đó chưa phải Completed và trạng thái mới
        // là Completed
        boolean wasCompleted = "Completed".equalsIgnoreCase(payment.getStatus().getStatusName());
        payment.setStatus(newStatus);
        payment = paymentRepository.save(payment);

        // Nếu thanh toán thành công (status_id = 3), cập nhật trạng thái booking sang
        // CONFIRMED
        if (!wasCompleted && newStatus.getPaymentStatusId() == 3) { // 3 = Completed
            ...
            try {
                User user = userRepository.findById(payment.getUser().getUserid())
                        .orElseThrow(() -> new RuntimeException("User not found"));
                List<BookingPassenger> passengers = bookingPassengerRepository
            ...
            System.out.println("DEBUG: Payment status is not Completed or was already completed. Current status: "
                    + newStatus.getStatusName() + " (ID: " + newStatus.getPaymentStatusId() + ")");
        }
        ...
        // Nếu thanh toán thành công (status_id = 3), kiểm tra và cập nhật trạng thái
        // lịch trình
    ...
    public String generateVietQr(String accountNo, String accountName, int amount, String phone)
